...
void place_animals(pAnimals anim_arr, int cur_anim_cnt, char an_type, int map_len, int an_health);
...
void generate_model(pAnimals anim_arr, struct Anim_counter anim_cnt, int map_len, int an_health) {
	...
	for (int i = 0; i < map_len; i++) {
		for (int j = 0; j < map_len; j++) {
			offset = j + i * map_len;
			(anim_arr + offset)->type = Animal_symbols[A_VOID];
			(anim_arr + offset)->x = j;
			(anim_arr + offset)->y = i;
		}
	}

	place_animals(anim_arr, anim_cnt.rab_cnt, Animal_symbols[A_RABBIT], map_len, an_health);
	place_animals(anim_arr, anim_cnt.wlf_m_cnt, Animal_symbols[A_WOLF_M], map_len, an_health);
	place_animals(anim_arr, anim_cnt.wlf_f_cnt, Animal_symbols[A_WOLF_F], map_len, an_health);
}
...
void place_animals(pAnimals anim_arr, int cur_anim_cnt, char an_type, int map_len, int an_health) {
	...
	while (cur_anim_cnt > 0) {
		for (int i = 0; i < map_len; i++) {
			for (int j = 0; j < map_len; j++) {
				offset = j + i * map_len;
				if ((cur_anim_cnt > 0) && ((anim_arr + offset)->type == Animal_symbols[A_VOID]) && (randomize(0, 100) > 99)) {
